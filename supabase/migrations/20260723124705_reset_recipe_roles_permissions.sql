-- Clean reset of recipe roles and permissions.
...
create policy "Users can update their own recipes or admins can update any recipe"
  on public.recipes
  for update
  using (
...
create policy "Users can delete their own recipes or admins can delete any recipe"
  on public.recipes
  for delete
  using (
    owner_id = auth.uid() or public.is_admin()
